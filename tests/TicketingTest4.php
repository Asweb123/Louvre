...
class TicketingTest4 extends WebTestCase
{
    ...
    public function testBooking()
    {
        ...
        $crawler = $client->followRedirect();
        //Control Pricing Process when PricingType = 2 (Half-Day) with all combinations pricing
        $this->assertCount(1, $crawler->filter('html:contains("23,00 €")'));

        $link = $crawler->selectLink('Retour')->link();
        $crawler = $client->click($link);
        //Control back link button to second ticketing step
        $this->assertCount(1, $crawler->filter('html:contains("Afin de")'));

        $link = $crawler->selectLink('Retour')->link();
        $crawler = $client->click($link);
        //Control back link button to first ticketing step where there is no "Retour" button
        $this->assertCount(0, $crawler->filter('html:contains("Retour")'));

        $link = $crawler->selectLink('Annuler la commande')->link();
        $crawler = $client->click($link);
